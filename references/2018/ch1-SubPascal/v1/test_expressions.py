...
from .subpascal import tokenize, parse, evaluate, Operator, repl
from .subpascal import UnexpectedEndOfInput, UnexpectedCloseParen
from .subpascal import UnknownSymbol, InvalidOperator
from .subpascal import MissingArgument, TooManyArguments
from .subpascal import NullExpression
...
def test_evaluate_unknown_operator():
    with raises(UnknownSymbol) as excinfo:
        expr = parse(tokenize("@"))
        evaluate(expr)
    assert str(excinfo.value) == "Unknown symbol: '@'."
